...
router.get('/playlist-earnings/:wallet', async (req, res, next) => {
  try {
    const wallet = normalizeWallet(req.params.wallet);
    const summary = await playlistService.getWalletPlaylistRevenueSummary(wallet);
    res.json(summary);
  } catch (err) {
    next(err);
  }
...
router.post('/set-royalty-splits', async (req, res, next) => {
  try {
    ...
    const total = (splits.artist || 0) + (splits.nft_holders || 0) + (splits.activity_pool || 0) + passiveTotal;
    ...
    for (const recipient of (splits.passive || [])) {
      const recipientProfile = profiles[normalizeWallet(recipient.wallet)];
...
router.post('/tip', async (req, res, next) => {
  try {
    const fromWallet = normalizeWallet(req.body?.from_wallet);
    const toWallet = normalizeWallet(req.body?.to_wallet);
    const { tip_type, amount_eth, dj_set_id, artist_splits, dj_percent } = req.body || {};

    if (!fromWallet || !tip_type || !amount_eth) {
    ...
    if (tip_type === 'artist') {
      if (!toWallet) return res.status(400).json({ error: 'Missing to_wallet for artist tip' });
      distribution.push({ wallet: toWallet, amount_eth: remaining, role: 'artist' });
    } else if (tip_type === 'dj') {
      ...
      distribution.push({ wallet: toWallet || set?.dj_wallet, amount_eth: djCut, role: 'dj' });
      ...
      if (artistPoolCut > 0 && artist_splits?.length) {
        ...
        for (const a of artist_splits) {
          distribution.push({
            wallet: normalizeWallet(a.wallet),
    ...
    const profiles = await profileService.loadProfiles();
    const tipper = profiles[fromWallet];
    const recognized = tipper && tipper.account_type !== null && profileService.isSubscriptionActive(tipper);

    logger.info({ tip_type, from_wallet: fromWallet, gross_eth: grossEth, platform_cut_eth: platformCut, distribution }, 'Tip processed');
...
router.post('/nft-sale-fee', async (req, res, next) => {
  try {
    const sellerWallet = normalizeWallet(req.body?.seller_wallet);
    const { sale_price_eth, nft_type, is_primary } = req.body || {};
    if (!sale_price_eth || !nft_type || !sellerWallet) {
    ...
    logger.info({ nft_type, seller_wallet: sellerWallet, sale_price_eth, platform_fee: platformFee }, 'NFT sale fee calculated');
    res.json({
      success: true,
