...
let currentPlaybackCid = null;

function initializeTelemetry(cid) {
  if (telemetryInterval) terminateTelemetry();
  currentPlaybackCid = cid;
  currentPlaybackSessionId = crypto.randomUUID
    ? crypto.randomUUID()
  ...
  telemetryInterval = setInterval(function () {
    ...
    fetch('/api/analytics/heartbeat', {
      ...
      body: JSON.stringify({
        cid: currentPlaybackCid,
        sessionId: currentPlaybackSessionId,
        elapsedSeconds: 10,
        isNewPlay: isFirstPing,
      }),
    }).catch(function (err) { console.debug('Telemetry error', err); });
    isFirstPing = false;
  }, 10000);
}

function terminateTelemetry() {
