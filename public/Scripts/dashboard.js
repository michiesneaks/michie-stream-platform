...
(function () {
  ...
  function renderPlaylists(profile) {
    ...
    if (!playlists.length) {
      panel.style.display = '';
      bodyEl.innerHTML = '<p class="text-muted small py-3">No playlists yet. <a href="playlists.html" style="color:var(--teal)">Create or manage playlists →</a> <span style="color:var(--text-muted)">You can still convert eligible favorites into playlists from Favorites.</span></p>';
      return;
    }
    ...
    bodyEl.innerHTML = playlists.slice(0, 6).map(function (pl) {
      ...
      var itemCount = pl.items ? pl.items.length : (pl.cids ? pl.cids.length : 0);
      var totals = (pl.analyticsView && pl.analyticsView.totals) || {};
      var top = (pl.analyticsView && pl.analyticsView.topAssets && pl.analyticsView.topAssets[0]) || null;
      var revenue = parseFloat(totals.collectedEth) || 0;
      return '<div class="pl-row">' +
        '<div>' +
          '<div class="pl-name">' + esc(pl.name || 'Untitled Playlist') + '</div>' +
          '<div class="pl-meta">' + itemCount + ' assets · ' + revenue.toFixed(4) + ' Ξ collected · ' + relativeTime(pl.updatedAt || pl.createdAt) + '</div>' +
          (top
            ? '<div class="pl-meta">Top asset: ' + esc(top.title || 'Untitled asset') + ' · ' + (parseFloat(top.revenueEth) || 0).toFixed(4) + ' Ξ</div>'
            : '<div class="pl-meta">No revenue-driving asset yet.</div>') +
        '</div>' +
        '<div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">' +
          '<span class="pl-badge ' + (isPublic ? 'pub' : 'priv') + '">' + (isPublic ? '🌐 Public' : '🔒 Private') + '</span>' +
          '<a href="playlists.html" style="color:var(--teal);font-size:11px;text-decoration:none;">Open →</a>' +
        '</div>' +
      '</div>';
    }).join('');
  }
